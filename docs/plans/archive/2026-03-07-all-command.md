# `veriresume all` Command Implementation Plan
...
**Files:**
- Create: `packages/veriresume-cli/src/commands/all.test.ts`
...
describe("runAll", () => {
  ...
  before(async () => {
    tempDir = await mkdtemp(path.join(tmpdir(), "veriresume-all-"));
...
Run: `cd packages/veriresume-cli && node --experimental-strip-types --test src/commands/all.test.ts`
...
**Files:**
- Create: `packages/veriresume-cli/src/commands/all.ts`
...
```bash
git add packages/veriresume-cli/src/commands/all.ts packages/veriresume-cli/src/commands/all.test.ts
...
Change version in `packages/veriresume-cli/package.json` to `"0.1.7"`.

**Step 3: Verify CLI works**

Run: `cd packages/veriresume-cli && node --experimental-strip-types src/index.ts all --help`
Expected: Shows "Run full pipeline: scan → infer → render → sign → pack → verify"

**Step 4: Commit**

```bash
git add packages/veriresume-cli/src/index.ts packages/veriresume-cli/package.json
