...
describe("scan-multi", () => {
  describe("discoverLocalRepos", () => {
    it("finds git repos in parent directory and ignores non-repos", async () => {
      const parentDir = path.join(os.tmpdir(), `skillproof-test-${Date.now()}`);
    ...
    it("returns empty for directory with no git repos", async () => {
      const parentDir = path.join(os.tmpdir(), `skillproof-empty-${Date.now()}`);
      await mkdir(parentDir, { recursive: true });
    ...
    it("finds nested git repos recursively", async () => {
      const parentDir = path.join(os.tmpdir(), `skillproof-nested-${Date.now()}`);
      await mkdir(parentDir, { recursive: true });
